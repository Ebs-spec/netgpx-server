...
if ($trackId && $userId) {
  ...
  switch ($type) {
    ...
    case "gpx":
      ...
      $xml->writeAttributeNs("xsi", "schemaLocation", NULL, "http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd https://github.com/Ebs-spec/netgpx-android/1 https://raw.githubusercontent.com/Ebs-spec/netgpx-server/master/scripts/gpx_extensions1.xsd");
      $xml->writeAttributeNs("xmlns", "xsi", NULL, "http://www.w3.org/2001/XMLSchema-instance");
      $xml->writeAttributeNs("xmlns", "netgpx", NULL, "https://github.com/Ebs-spec/netgpx-android/1");
          ...
          $xml->startElement("trkpt");
            ...
            if (!is_null($position->speed) || !is_null($position->bearing) || !is_null($position->accuracy) || !is_null($position->provider)) {
              ...
              if (!is_null($position->speed)) {
                $xml->writeElementNS("netgpx", "speed", NULL, $position->speed);
              }
              if (!is_null($position->bearing)) {
                $xml->writeElementNS("netgpx", "bearing", NULL, $position->bearing);
              }
              if (!is_null($position->accuracy)) {
                $xml->writeElementNS("netgpx", "accuracy", NULL, $position->accuracy);
              }
              if (!is_null($position->provider)) {
                $xml->writeElementNS("netgpx", "provider", NULL, $position->provider);
              }
